            ...
            if (token.length == 0){
                await saveNotifications([
                    [
                        8,
                        userId,
                        `Bạn được phân công nhận xe ${registry.license_plate} lúc ${registry.car_delivery_time.slice(0,5)} ngày ${registry.date.split("-").reverse().join("/")} tại ${registry.address}`, registry.id, 
                        0,
                    ]
                ])
            }else {
...
const sendToCustomerCompletedRegistry = (registry_id,type) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const data = handleDataMessage(token, type == 0 ?  "FAIL_REGISTRY" : "COMPLETED_REGISTRY", registry.license_plate);
            const notification = convertDataToArray(token,  type == 0 ?  "FAIL_REGISTRY" : "COMPLETED_REGISTRY", registry);
            await saveNotifications(notification);
            await sendNotifications();
            await updateSendStatus();
            await changeNotificationStatus();
                ...
                if (token.length == 0){
                    await saveNotifications([
                        [
                            9,
                            user.id,
                            `Xe ${registry.license_plate} đã hoàn thành phí và lệ phí`, registry.id, 
                            0,
                        ]
                    ])
                }else {
        ...
        case "EXPIRED": {
            data.forEach((a) => {
                ...
                result.push([
                    1,
                    a.user_id,
                    `Xe ${a.license_plate} của bạn sẽ hết hạn đăng kiểm vào ngày  ${date.getDate()}/${
                        date.getMonth() + 1
                    }/${date.getFullYear()}.`,
                    a.license_plate,
                    a.registry_id,
        ...
        case "ASSIGNMENT": {
            data.forEach((a) => {
                ...
                result.push([
                    8, 
                    a.user_id, 
                    `Bạn được phân công nhận xe ${props.license_plate} lúc ${props.car_delivery_time.slice(0,5)} ngày ${props.date.split("-").reverse().join("/")} tại số ${props.address}`, 
                    props.license_plate,
                    props.id, 
                    0]);
            });
