...
import {converLicensePlate, reConvertLicensePlate} from '../../services/utils/index'
import config from '../../config'
import {convertAlert} from '../../component/shared/ConvertAlert'
import { CommonActions } from '@react-navigation/native';

const CarUpdateScreen = ({navigation,route}) => {
                ...
                CarService.get_carInfor_byId(carId)
                    .then((res) =>{
                        if (res.status==1){
                            ...
                            setLicensePlates(converLicensePlate(res.data.license_plates))
                            setYear(res.data.manufacture_at)
                            setListImage(res.data.display_images)
                            setIsLoading(false);
    ...
    const deleteListImage = (imageId,index) =>{
        setDeleteList([...deleteList,imageId])
        let list = listImage
        delete list[index]
    }
    React.useEffect(() =>{
    ...
    const handleUpdate = () =>{
        ...
        console.log(data);
        CarService.update_car_byId(carId,data)
            .then((res) =>{
                if (res.status == 1 ){
                    toast.hideAll();
                    toast.show(
    ...
    return (
        <View style={{flex:1}}>
            <Header onGoBack={()=>navigation.goBack()} title='Sửa thông tin xe'/>
            {
                isLoading == true ?
                (
                    <Loading />
                                ...
                                <View style={styles.photo_group}>
                                    {
                                        listImage?.map((image,index)=>(
                                            <View style={styles.photo_group_items} key={image.id}>
                                                <Image 
                                                ...
                                                />
                                                <TouchableOpacity style={styles.btn_delete} onPress={()=>deleteListImage(image.id,index)}>
                                                    <Image 
