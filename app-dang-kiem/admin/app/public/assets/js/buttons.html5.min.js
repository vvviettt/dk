(function (g) {
  ...
  function E(a, b) {
    v === o &&
      (v =
        -1 === y.serializeToString(g.parseXML(F['xl/worksheets/sheet1.xml'])).indexOf('xmlns:r'));
      ...
      if (g.isPlainObject(c)) {
        ...
        if (v) {
          ...
          for (h = l.length; e < h; e++)
            (n = c.createAttribute(l[e].name.replace(':', '_dt_b_namespace_token_'))),
              (n.value = l[e].value),
  ...
  function N(a, b) {
    ...
    for (
      ...
      f < e && !((c = a.body[f][b].toString().length), c > d && (d = c), 40 < d);
      f++
    );
    return 5 < d ? d : 5;
  }
  ...
  r.ext.buttons.excelHtml5 = {
    ...
    action: function (a, b, d, c) {
      var f = 0,
        ...
        d = function (a) {
          ...
          for (var b = 0, c = a.length; b < c; b++) {
            ...
            'number' === typeof a[b] ||
            (a[b].match && g.trim(a[b]).match(/^-?\d+(\.\d+)?$/) && !g.trim(a[b]).match(/^0\d+/))
              ? (d = p(e, 'c', { attr: { t: 'n', r: d }, children: [p(e, 'v', { text: a[b] })] }))
              : ((i = !a[b].replace
                  ? a[b]
      ...
      for (k = b.header.length; m < k; m++)
        d.appendChild(
          p(e, 'col', { attr: { min: m + 1, max: m + 1, width: N(b, m), customWidth: 1 } })
        );
